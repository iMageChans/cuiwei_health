...
export default function ExercisesBase(data:any) {
  const [baseData, setBaseData] = useState<ExercisesModel[]>([]);

  const [page, setPage] = useState(1);
  const [total, setTotal] = useState(0);
  const title = data.params.body_part.charAt(0).toUpperCase() + data.params.body_part.slice(1)

  useEffect(() => {
    const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'https://heartwellness.app'; // 你可以设置一个默认值
    if (page === 1) setBaseData([]);
    // 发送 GET 请求
    axios.get(apiUrl + `/api/fitness/exercises/body-parts/${data.params.body_part}/?page=${page}&page_size=20`)
      .then(response => {
          ...
          <div className="mx-auto lg:px-40 px-4">
            <div className="max-w-7xl mx-auto">
              <h1 className='text-xl md:text-lg lg:text-40px font-bold text-black text-center'>
                Best {title} Moves To Add To Your Routine
              </h1>
              <p className="text-center text-base md:text-lg lg:text-2xl text-black opacity-70 mt-5 lg:mt-10">
               Build A Solid Foundation With These Targeted {title} Exercises. <br/>Perfect For All Levels, Designed To Improve Your Form And Maximize Progress.
              </p>
              <List
                ...
                renderItem={(item) => (
                  <Link
                    href={ `/exercises/${item.body_part.slug}/${item.slug}`}
                    key={item.id}>
                    <List.Item >
